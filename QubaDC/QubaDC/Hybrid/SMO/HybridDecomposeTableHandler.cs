...
namespace QubaDC.Separated.SMO
{
    class HybridDecomposeTableHandler
    {
        ...
        public HybridDecomposeTableHandler(DataConnection c, SchemaManager schemaManager,SMORenderer renderer)
        {
        ...
        internal void Handle(DecomposeTable partitionTable)
        {
            ...
            con.DoTransaction((transaction, c) =>
            {
                ...
                String insertFromTable = SMORenderer.RenderInsertFromOneTableToOther(originalTable.Table, firstTableSchema, null,
                    firstTableSchema.ColumnDefinitions.Select(x => x.ColumName).ToArray(),
                    firstTableSchema.ColumnDefinitions.Select(x => x.ColumName).ToArray());
                ...
                String insertFromSeconDtable = SMORenderer.RenderInsertFromOneTableToOther(originalTable.Table, secondTableSchema, null,
                    secondTableSchema.ColumnDefinitions.Select(x => x.ColumName).ToArray(),
                    secondTableSchema.ColumnDefinitions.Select(x => x.ColumName).ToArray());
                con.ExecuteNonQuerySQL(insertFromSeconDtable);


                String deleteFromFirstTable = this.SMORenderer.CRUDRenderer.RenderDelete(originalTable.Table.ToTable(), null);
