...
{
    public class SeparatedQSSelectExecuter
    {
        ...
        )
        {
            ...
            DataConnection.AquiereOpenConnection(con =>
            {
                ...
                if (currentSchema.ID != expectedSchema.ID)
                {
                    throw new InvalidOperationException("Expected Schemaversion has changed - hist potentially not actual, abortin., expected: " + expectedSchema.ID + " got: " + currentSchema.ID);
                }
