...
{
    public class SeparatedQSSelectHandlerUsingHist : QueryStoreSelectHandler
    {
        public override QueryStoreSelectResult HandleSelect(SelectOperation s, SchemaManager schemaManager, DataConnection con, TableMetadataManager metaManager, CRUDVisitor cRUDHandler,QueryStore qs)
        {
            ...
            TableToLock[] aliasTables = s.GetAllSelectedTables()                                             
                                        .Select(x => {
                                            ...
                                            var orignalTable = new Table() { TableName = x.TableName, TableSchema = x.TableSchema };
                                            var res = new TableToLock { Alias =x.TableAlias, LockAsWrite = false, Name = cRUDHandler.CRUDRenderer.PrepareTable(orignalTable) };
                                            return res;
    ...
    ,
                ...
                select = cRUDHandler.RenderSelectOperation(s);
                RewrittenSerialized = JsonSerializer.SerializeObject(s);

                selectHash = cRUDHandler.RenderHashSelect(s);
                 selectHashSerialized = cRUDHandler.RenderHashSelect(s);
        ...
        public override QueryStoreReexecuteResult ReExecuteSelectFor(Guid gUID, QueryStore qs,DataConnection con, CRUDVisitor cRUDHandler, SchemaManager schemaManager)
        {
            ...
            String original = row.Field<String>("ReWrittenQuerySerialized");
            SelectOperation basic = JsonSerializer.DeserializeObject<SelectOperation>(original);
            ...
            SelectOperation modified = something(basic, schemaAtExecutionTime, queryTime);

            String querySerialized = row.Field<String>("HashSelect");


            query = cRUDHandler.RenderSelectOperation(modified);
            querySerialized = cRUDHandler.RenderHashSelect(modified);
        ...
        public SelectOperation something(SelectOperation s, SchemaInfo schema, DateTime queryTime)
        {
