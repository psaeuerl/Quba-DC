...
{
    public class QSUpdateSOmeSectionValidationRunner
    {
        public void run(MySQLDataConnection con, SystemSetup system, String dbName, int numberQueries, int sectionsToUpdate, int howOftenToUpdate,  Boolean addIndexHist)
        {
            ...
            int[] sectionsToUpdateIds = ids.Take(sectionsToUpdate).ToArray();
            for(int i=0;i< numberQueries;i++)
            {
                UpdateOperation uo = new UpdateOperation()
                {
                    ...
                    {
                        LHS = new Restrictions.ValueRestrictionOperand() { Value = "Section" },
                        RHS = new Restrictions.ValueRestrictionOperand() { Value = String.Join(", ",sectionsToUpdateIds)},
